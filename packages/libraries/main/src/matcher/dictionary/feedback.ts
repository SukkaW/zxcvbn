...
const getDictionaryWarningNames = (options: Options, isSoleMatch?: boolean) => {
  if (isSoleMatch) {
...
const getDictionaryWarning = (
  ...
  if (dictName === 'passwords') {
    ...
    warning = getDictionaryWarningWikipedia(options, isSoleMatch)
  } else if (isAName) {
    warning = getDictionaryWarningNames(options, isSoleMatch)
